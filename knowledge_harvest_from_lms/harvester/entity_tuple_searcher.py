...
class EntityTupleSearcher:
    ...
    def search(self, weighted_prompts, max_word_repeat, max_ent_subwords, n):
        ...
        for t in range(max_ent_subwords**n_ents):
            n_masks = get_n_masks(t=t, n_ents=n_ents, max_ent_subwords=max_ent_subwords)
            (f'searching with n_masks={n_masks}')
    ...
    def dfs(
        ...
        for ent_logprob, pred_ent in collected_ents:
            ...
            if (
                len(collected_tuples_heap) == n
                and min_upd < collected_tuples_heap[0][0]
            ):
                break
    ...
    def dfs_ent(
        ...
        if len(cur_token_ids) == n_masks[ent_idx]:
            ...
            if any([word in stopword_list for word in pred_ent.split()]):
                return
        ...
        for logprob, pred_id in zip(logprobs, pred_ids):
            ...
            if (
                len(collected_ent_heap) == n
                and min_logprob_upd < collected_ent_heap[0][0]
            ):
                break
            ...
            if not any([ch.isalpha() for ch in self._model.tokenizer.decode(pred_id)]):
                continue

            if any(
                [
                    punc in self._model.tokenizer.decode(pred_id)
