...
from knowledge_harvest_from_lms.models import LLMResource
from knowledge_harvest_from_lms.models.gpt3 import GPT3
from knowledge_harvest_from_lms.models.ollama_model import OllamaModel
...
def get_paraphrase_prompt(llm_resource: LLMResource, prompt: str, ent_tuple: tuple):
    ...
    for _ in range(5):
        raw_response = llm_resource.call(prompt=f'paraphrase:\n{sent}\n')
        para_sent = raw_response.text
...
def search_prompts(
    ...
    seed_ent_tuples: list[tuple[str]],
    ...
    while True:
            ...
            for ent_tuple in seed_ent_tuples:
                ...
                if request_str not in cache or prompt in init_prompts:
                    cache[request_str] = get_paraphrase_prompt(
                        llm_resource=llm_resource, prompt=prompt, ent_tuple=ent_tuple
                    )

                para_prompt = cache[request_str]
                print(
                    f'prompt: {prompt}\tent_tuple: {ent_tuple}'
                    f'\t-> para_prompt: {para_prompt}'
                )

                if (
                    para_prompt is not None
                    and para_prompt not in init_prompts + prompts
                ):
        ...
        if len(new_prompts) == 0:
            ...
            for new_prompt in sorted(new_prompts, key=lambda t: len(t)):
                if len(prompts) != 0:
                    max_sim = max(
                        [fuzz.ratio(new_prompt, prompt) for prompt in prompts]
                    )
                    print(f'-- {new_prompt}: {max_sim}')
                if (
                    len(prompts) == 0
                    or max([fuzz.ratio(new_prompt, prompt) for prompt in prompts])
...
def main(rel_set='conceptnet', similarity_threshold=75, model='gpt3'):
    if model.lower() == 'gpt3':
        llm_resource = GPT3()
    else:
        llm_resource = OllamaModel(model=model)
    print(llm_resource)
    rel_info_path = Path(__file__).parent / "relation_info"
    relation_info = json.load(open(rel_info_path / f'{rel_set}.json'))
    ...
    for rel, info in relation_info.items():
        ...
        output_path = rel_info_path / f'{rel_set}.json'
        json.dump(relation_info, open(output_path, 'w'), indent=4)


def fire_search_promts():
    """Execute search_prompts:main as a CLI method with `fire`."""
    fire.Fire(main)
